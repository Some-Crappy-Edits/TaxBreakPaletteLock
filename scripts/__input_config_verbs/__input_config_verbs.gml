...
function __input_config_verbs()
{
    return {
        keyboard_and_mouse:
        {
			...
			menuaction: input_binding_key(ord("F")),
            //cancel:  input_binding_key(vk_backspace),
			action: input_binding_key(ord("F")),
            skiptext:  input_binding_key(vk_enter),
			fasttext: input_binding_key(vk_shift),
			...
			ignore: input_binding_key(vk_home), //Useless key to maintain array in SettingsMenu
        },
        ...
        gamepad:
        {
			...
			menuaction: input_binding_gamepad_button(gp_face1),
			action: input_binding_gamepad_button(gp_face2),
            skiptext:  input_binding_gamepad_button(gp_face3),
			fasttext:  input_binding_gamepad_button(gp_shoulderlb),
            run: input_binding_gamepad_button(gp_shoulderlb),
			
			ignore: input_binding_empty(),
