...
//Can you tell I used a tutorial for this? :)

//Colli is stationary/main collision, Runii is moving semisolid, Deadli is death collision 
//Lazii is stationary semisolid, and Molli is broken but is suppose to be moving solid (ie. Thwomp)

//Currently, Ally has a bug where she jitters on moving semisolids, changing subPixel did not fix this
//It might be tolerated for final release
...
//Crouching
	//Transition to Crouch
		//Manual
		//if OnGround && (downKey || place_meeting(x,y,Colli)) {
			//crouching = true;
		//};
		
		//Forced Crouch
		//if OnGround && place_meeting(x,y,Colli)
		//{
			//crouching = true;
		//};
...
//Pause Function
if global.PauseState = true && OnGround = true {
	setOnGround(true)
	moveDir = 0;
	jumpKey = 0;
	jumpKeyPressed = 0;
...
//Dialogue Face
if global.PauseState = true && instance_exists(Blabber) {
	face = global.NPCface;
};
	...
	if crouching { xspd = moveDir * crawlSpd };

	//X Collision
	var _subPixel = .4; //.5 normally, seems like due to Ally's size, the value is too high and causes jitter effect, also, changing other subpixels to match seems to break it, so dont do that
...
//Y Movement
	...
	if jumpKeyBuffered && !downKey && jumpCount < jumpMax && (!downKey || _floorIsSolid)
	{
		//Delay Reset
		jumpKeyBuffered = false;
		jumpKeyBufferTimer = 0;
		
		//Double Jump Potential
		jumpCount++;
...
//Death
if rip {sprite_index = deathSpr};

//Mask Collision
mask_index = maskSpr;
if crouching{mask_index = crouchSpr};
if rip{mask_index = deathSpr};
